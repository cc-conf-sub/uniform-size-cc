...
public class DNode {
    ...
    private static int findMinSizeBound(ArrayList<Integer> cluster, ArrayList<Integer> size_bounds) {
        int min_val = size_bounds.get(cluster.get(0));
        for (int node : cluster) {
            int cur_val = size_bounds.get(node);
            if (cur_val < min_val)
                min_val = cur_val;
        }
        return min_val;
    ...
    public static ArrayList<ArrayList<Integer>> nonuniform_random_node_network(ArrayList<ArrayList<Integer>> prob_matrix, ArrayList<Integer> size_bounds, boolean order) {
        ...
        */
        ...
        while (settled_nodes.size() < num_nodes) {
            ...
            if (max_cluster[0] >= 0.5) {
                // add cur_node to the clustering
                int new_index = (int) Math.round(max_cluster[1]);
                cur_clustering.get(new_index).add(cur_node);
                settled_nodes.put(cur_node, new_index);
                //if (cur_size_limit < cluster_size_limits.get(new_index));
                //    cluster_size_limits.set(new_index, cur_size_limit); // update size
                cluster_size_limits.set(new_index, findMinSizeBound(cur_clustering.get(new_index), size_bounds));
                //int min_bound = findMinSizeBound(cur_clustering.get(new_index), size_bounds);
                //if (cur_clustering.get(new_index).size() > min_bound)
                //    System.out.println("WARNING! Cluster " + new_index + " has size " + cur_clustering.get(new_index).size() + " and true limit " + min_bound + " and reported limit " + cluster_size_limits.get(new_index));
